...
import { Calendar, Download, ExternalLink, Check, Info } from "lucide-react";
...
interface JobCardProps {
  job: ScrapingJob;
  onViewData: (jobId: number) => void;
  onDownload: (jobId: number) => void;
}

const ScraperJobCard: React.FC<JobCardProps> = ({ job, onViewData, onDownload }) => {
  ...
  const getInterestBadge = (interest: string) => {
    ...
    return (
      <Badge className={`${classes[interest as keyof typeof classes] || ""} px-3 py-0.5 rounded-full`}>
        {interest} interest
      </Badge>
    );
  };
