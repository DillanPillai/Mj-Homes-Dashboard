...
def clean_data(raw_data) -> pd.DataFrame:
    ...
    # Drop rows with too many missing values (>=30% NaN -> drop)
    df.dropna(thresh=int(0.7 * len(df.columns)), inplace=True)

    # Remove duplicates
    df.drop_duplicates(inplace=True)

    # --- Robust numeric parsing (avoid astype(float) on empty strings) ---

    # Convert rent_price to float (strip non-digits, then coerce)
    if "rent_price" in df.columns:
        ...
        df["rent_price"] = pd.to_numeric(rp, errors="coerce")

    # Convert floor_area to float (strip non-digits, then coerce)
    if "floor_area" in df.columns:
        ...
        df["floor_area"] = pd.to_numeric(fa, errors="coerce")
    ...
    # Fill missing suburb with 'Unknown' and normalise case
    if "suburb" in df.columns:
        df["suburb"] = (
            df["suburb"].fillna("Unknown").astype(str).str.strip().str.title()
        )
    ...
    # Final cleanup: fill remaining NaNs in numerics with 0 to keep downstream safe
    numeric_cols = [
        c for c in ["rent_price", "floor_area", "bedrooms", "bathrooms"] if c in df.columns
    ]
    if numeric_cols:
...
# Used for model training + prediction
def prepare_features(df: pd.DataFrame, valid_suburbs: list[str] | None = None) -> pd.DataFrame:
    """
    ...
    df = df[
        ~df["suburb"].isin(
            ["", "String", "??", "N/A", "Na", "Null", "None", "Unknown"]
        )
    ]
